        ...
        <div class="flex flex-col w-full md:flex-row flex-grow mt-5 mb-5 main-p">
            ...
            >
                <li v-for="(item, index) in categoryList" :key="index">
                    <NuxtLink
                        :class="{ active: category == item.id ? true : false }"
                        :to="`/article/${item.id == category ? '' : item.id}`"
                        >{{ item.name }}</NuxtLink
                    >
                </li>
            </ul>
            <div class="list flex flex-col justify-start items-start overflow-hidden flex-wrap">
                <NuxtLink
                    :to="`/article/view/${topic.id}`"
                    v-for="(topic, index) in treadList"
                    :key="index"
                    class="item flex flex-col md:flex-row md:pl-5 pb-5 overflow-hidden"
                >
                    <div class="flex flex-col justify-center items-center w-full md:w-40 md:mr-5">
