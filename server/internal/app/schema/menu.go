...
type Menu struct {
	...
	ParentRouter string      `json:"parent_router"`                          // 父级路径
...
type MenuQueryParam struct {
	...
	PrefixParentRouter string   `form:"-"`          // 父级路径(前缀模糊查询)
...
func (a Menus) SplitParentIDs() []uint64 {
	...
	for _, item := range a {
		if _, ok := mIDList[item.ID]; ok || item.ParentRouter == "" {
			continue
		}

		for _, pp := range strings.Split(item.ParentRouter, "/") {
...
func (a Menus) ToTree() MenuTrees {
	...
	for i, item := range a {
		list[i] = &MenuTree{
			...
			ParentRouter: item.ParentRouter,
...
type MenuTree struct {
	...
	ParentRouter string      `yaml:"-" json:"parent_router"`                       // 父级路径
