...
func (a *MenuRepo) Query(ctx context.Context, params schema.MenuQueryParam, opts ...schema.MenuQueryOptions) (*schema.MenuQueryResult, error) {
	...
	if v := params.PrefixParentRouter; v != "" {
		db = db.Where("parent_router LIKE ?", v+"%")
	}
...
func (a *MenuRepo) UpdateParentRouter(ctx context.Context, id uint64, ParentRouter string) error {
	result := GetMenuDB(ctx, a.DB).Where("id=?", id).Update("parent_router", ParentRouter)
	return errors.WithStack(result.Error)
}
